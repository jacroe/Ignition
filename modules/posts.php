...
function ign_posts_get($num = -1, $year = null, $month = null)
{
	...
	foreach (array_reverse(glob(IGN_PATH."data/posts/$searchString.json")) as $post)
	{
		...
		$posts[] = array("title"=>$jsonPost->title, "author"=>$jsonPost->author, "slug"=>$slug, "date"=>$jsonPost->date, "timeAgo"=>ign_timeAgo($jsonPost->date), "loc"=>$jsonPost->loc, "excerpt"=>$jsonPost->excerpt, "article"=>$jsonPost->article, "type"=>$jsonPost->type, "tags"=>$jsonPost->tags, "link"=>$jsonPost->link);
